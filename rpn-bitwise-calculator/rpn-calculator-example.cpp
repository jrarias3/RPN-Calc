 ...
 */
#define INPUT_CSV_FILE "/Users/robertarias/Desktop/rpn-bitwise-calculator/rpn-input.csv"
...
shared_ptr<uint16_t> rpn_calc(command const cmd, uint16_t const value = 0) {
    ...
    {
    case cmd_enter:
        my_stack.push(make_shared<uint16_t>(value));
        result = my_stack.top();
        break;
    ...
    case cmd_left_shift:
        ...
        if (my_stack.size() >= 2)
        {
            ...
            my_stack.pop();
            uint16_t result_value = top_value1 | top_value2;
